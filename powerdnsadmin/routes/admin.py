...
def _render_rrset_acl_badge(rrset_acls):
    """Return a pre-rendered HTML snippet for the Scoped column."""
    if not rrset_acls:
        return '<span class="text-muted">&mdash;</span>'
    tooltip = ' | '.join(
        '{0} {1}'.format(r.record_name_pattern, r.record_type)
        for r in rrset_acls
    )
    return (
        '<span class="badge badge-warning" data-toggle="tooltip" title="{title}">'
        '{count} rule(s)</span>'
    ).format(title=tooltip, count=len(rrset_acls))
    ...
    * ``order[0][column]`` / ``order[0][dir]`` — single-column sort on
    ...
    for key in keys:
        ...
        rows.append([
            ...
            _render_rrset_acl_badge(key.rrset_acls),
            actions_html,
        ])
